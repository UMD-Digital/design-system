...
const CreateAnimationArrowContainer = (props: TypeArrowProps) => {
  const { id, wrapperStyleRef } = props;
  const SVG_ANIMATION_ARROW = `<svg id="${id}" class="aninmation-arrow" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 973.01 976"><defs><style>.arrow-video{fill:#e21833;fill-rule:evenodd;}</style></defs><path class="arrow-video" d="m427.44,497.01L0,0h545.57l427.44,497.01-411.96,478.99H15.48l411.96-478.99Z"/></svg>`;

  if (!wrapperStyleRef) {
    return new ElementBuilder().withHTML(SVG_ANIMATION_ARROW).build();
  }

  return new ElementBuilder()
    .withClassName(wrapperStyleRef)
    .withHTML(SVG_ANIMATION_ARROW)
    .withStylesIf(
      wrapperStyleRef === 'animation-overlay-layer-red-arrow-container',
      {
        element: {
          position: 'absolute',
          top: '50%',
          ...
          display: 'flex',
          alignItems: 'center',
          '& .aninmation-arrow': { height: '100%' },
        },
      },
    )
    .withStylesIf(
      wrapperStyleRef === 'animation-overlay-layer-black-arrow-container',
      {
        element: {
          position: 'absolute',
          top: '50%',
...
export const createAnimationChevronFlow = ({
  ...
  (() => {
    ...
    return containerBuilder
      .withChild(overlay)
      .withChild(overlayArrow)
      .withChild(slideInArrow)
      .withChild(slideInArrowBlackOne)
      .withChild(slideInArrowBlackTwo)
