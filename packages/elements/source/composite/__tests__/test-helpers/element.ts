...
export function validateElementStructure(
  ...
  if (expectedConfig.hasElement !== false) {
    ...
    if (expectedConfig.tagName) {
      expect(result.element.tagName.toLowerCase()).toBe(expectedConfig.tagName.toLowerCase());
    }
    
    if (expectedConfig.className) {
      expect(result.element.classList.contains(expectedConfig.className)).toBe(true);
    }
  }
 ...
 */
export function createMockElementModel(config: TestElementConfig = {}): ElementModel {
  return {
