...
const ANIMATION_NAME = 'card-carousel-slide-in';
...
const CreateIntro = (props: TypeCarouselCardsRequirements) => {
  ...
  const createHeadline = () => {
    ...
    return new ElementBuilder(headline)
      ...
      .styled(typography.sans.largest)
      .withStyles({
        element: {
...
export const createCompositeCarouselCards = (props: TypeCarouselCardsRequirements) =>
  (() => {
    ...
    if (shadowRef) {
      shadowRef.style.minHeight = '360px';
    }
    ...
    const carouselContainer = carouselElements.blocks.CreateElement({
      ...
      blocks: props.cards,
      mobileBreakpoint: token.media.breakpointValues.large.min,
      ...
      showHint: false,
      button: {
        ...carouselElements.buttonColorsOnBlack(),
        outsetOffset: '52px',
      },
    });

    const carouselWrapperModel = new ElementBuilder()
      .withClassName('carousel-cards-block-wrapper')
      .withStyles({
        element: {
          [`@container (${token.media.queries.desktop.min})`]: {
            width: '60%',
    ...
    const wrapperModel = new ElementBuilder()
      ...
      .withChildren(introModel, carouselWrapperModel)
      .build();
    ...
    const containerModel = new ElementBuilder()
        ...
        element: {
          ...
          '& > svg': {
            ...
            ...withViewTimelineAnimation({
              animation: `${ANIMATION_NAME} ease-in-out forwards`,
      ...
      .withModifier((element) => {
        element.insertAdjacentHTML('afterbegin', BACKGROUND_TEXTURE);
      })
      .build();

    containerModel.styles += KEYFRAMES;
