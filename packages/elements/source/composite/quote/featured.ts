...
import InlineQuote, { InlineProps } from './inline';
import * as Utility from 'utilities';
import { ElementModel } from 'model';
import { ElementVisual } from '_types';
import { CreateAction, MEDIUM, SMALL, BaseProps, AnimateQuote } from '.';
import CreateQuoteImageContainer from './elements/image';

interface QuoteFeatured extends InlineProps, BaseProps {}
...
const createImageContainer = (props: QuoteFeatured) => {
  ...
  const getContainerChildren = () => {
    ...
    if (image) {
      const imageElement = CreateQuoteImageContainer(props, image);
...
export default (props: QuoteFeatured) => {
